/**
 * 모든 문서 변경은 커맨드를 통과한다 (스테이지는 한 번에 한 페이지만 렌더한다).
 *
 * 커맨드는 `PDFCanvasDoc -> PDFCanvasDoc | null` 순수 함수이고, null은 "할 일 없음"이다.
 * 모든 변경을 한 지점으로 모으는 덕분에, 각 컴포넌트가 따로 신경 쓰지 않아도
 * undo/redo·dirty 추적·자동저장이 성립한다.
 *
 * 커맨드는 **바뀐 가지에 대해서만 새 객체**를 돌려준다. 그래서 히스토리 항목이 문서 전체 참조를
 * 싸게 들고 있을 수 있다 (../store/history.ts 참고).
 */
import type { PDFCanvasDoc } from '../model/types'

/** 다음 문서를 돌려준다. 변경이 없으면 null. */
export type Command = (doc: PDFCanvasDoc) => PDFCanvasDoc | null

export interface LabeledCommand {
  label: string
  run: Command
}

/** `updatedAt` 을 갱신한다. 헬퍼마다가 아니라 커밋되는 커맨드마다 한 번 적용한다. */
export function touch(doc: PDFCanvasDoc): PDFCanvasDoc {
  return { ...doc, updatedAt: new Date().toISOString() }
}

/** 인덱스로 한 페이지만 교체한다. 나머지 페이지는 참조가 그대로 유지된다. */
export function replacePage(
  doc: PDFCanvasDoc,
  index: number,
  map: (page: PDFCanvasDoc['pages'][number]) => PDFCanvasDoc['pages'][number],
): PDFCanvasDoc | null {
  const page = doc.pages[index]
  if (!page) return null
  const next = map(page)
  if (next === page) return null
  const pages = [...doc.pages]
  pages[index] = next
  return { ...doc, pages }
}

export * from './doc'
export * from './pages'
export * from './objects'
