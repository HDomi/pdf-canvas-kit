...
| **D30** | **소비자 예제 앱을 레포에 둔다 — 별칭 없이 `exports` 맵으로 해석한다** (2026.08.21) | `demo/` 는 별칭으로 소스를 직접 보므로 **패키징 경로를 검증하지 못한다.** R10·R11 에서 잡은 버그 네 개(`postinstall` 누락 파일, `SlotMap` 반공변, `createPDFCanvasEditor` export 누락, React `ref` 미충전)가 전부 그 경로에서만 드러났고, 그때 쓴 검증 앱은 레포 밖에 있어 **다음 세션에 사라졌다.** `examples/*` 를 workspace 로 두고 `npm run dev` 가 셋을 함께 띄운다 | (a) `demo/` 에 별칭 없는 페이지 추가: 한 vite 설정 안에서 별칭 유무를 섞으면 어느 쪽이 무엇을 보는지 흐려진다 (b) 매번 임시 앱 생성: 실제로 그렇게 했고 네 번 중 두 번을 놓쳤다 (c) CI 전용 검증: CI 가 없다(D17). 상세 20.22 |
| **D31** | **커스터마이징은 토큰 → `@layer` → 다이얼로그 위임 3단계. 크롬 슬롯은 만들지 않는다** (2026.08.21) | 실제 수요는 "우리 앱 모달과 생김새가 다르다" 였고 그건 UI 를 교체하는 것이 아니라 **동작만 함수로 받으면** 끝난다. 스타일은 `@layer` 로 소비자가 특이도 싸움 없이 이기게 하면 규칙 단위 커스터마이징까지 열린다. 크롬 슬롯은 조각 props 가 전부 `ReadSignal<T>` 라 조각마다 스냅샷 타입 + 구독 배선이 필요하고, 10개면 컨트롤러 표면 절반을 공개하는 것과 같다 | (a) D27 의 슬롯 전면 구현: 위 비용을 내고도 실제 수요는 3단계로 이미 해결된다 (b) 토큰만 확장: 배치·정렬처럼 값으로 표현할 수 없는 커스터마이징이 막힌다 (c) `!important` 로 이기라고 안내: 우리가 선택자를 하나 늘리면 조용히 깨진다. 상세 20.24 |
| **D32** | **문구·아이콘을 prop 으로 받는다. 아이콘은 글리프 → vanilla 노드 → 컴포넌트 3경로** (2026.08.21) | 호스트가 번역해야 한다. `configureStrings` 전역 함수만으로는 "이 편집기의 문구" 를 줄 수 없고, prop 이 라이브러리 관례다(AG Grid `localeText`). 아이콘은 문자열만으로 부족하다 — SVG·프레임워크 컴포넌트를 넣어야 하므로 커스텀 객체와 **같은 3경로 패턴**을 쓴다 | (a) 전역 `configureStrings` 만: 인스턴스 개념이 없고 "이 컴포넌트만 영어" 가 불가능하다 (b) SVG 스프라이트 내장: 교체 수단이 따로 필요해지고 아이콘 프레임워크를 요구하게 된다 (c) 문구를 반응형으로: 80곳의 렌더 경로를 signal 로 바꿔야 하고, 실제 요구는 부팅 시 1회 설정이다. 상세 20.25 |
...
| **R12** ✅ | 커스터마이징 | **D27 을 정정하고 D31 로 갔다** — 토큰 75개 · `@layer` · 다이얼로그 위임 ✅. `/checks/` **299 케이스 / 44 그룹**. 상세 20.24 |
...
### 20.18 크롬 슬롯 설계 메모 (R12 — 2026.08.20 결정, 미착수)

#### 왜 지금 구조에서 싸게 되는가

렌더 층을 만들 때 각 조각이 **컨트롤러를 통째로 받지 않고 좁은 props 인터페이스**를 받게 했다.
...
그래서 교체 지점이 이미 잘려 있다. R12 가 할 일은 새로 자르는 게 아니라 **이 인터페이스들을
공개 타입으로 승격하고 `editorShell` 에 분기를 넣는 것**이다. R9 의 portal 배선
(`onMountCustom` 프로토콜, §17.2)도 그대로 재사용한다 — 컨테이너를 만들고 알리는 방식이 같다.

#### 경계 — 무엇을 열고 무엇을 닫는가

| 조각 | 슬롯 | 근거 |
| --- | --- | --- |
| `topBar` `toolbar` `pageThumbList` `stageControls` `pageMeta` `saveBadge` `emptyState` | ✅ | 컨트롤러 호출만 한다. 좌표를 모른다 |
| `confirmDialog` `uploadDialog` `pageContextMenu` | ✅ **수요 1순위** | 호스트 앱에 자기 모달이 이미 있다 |
| `inspector` | ⚠️ 부분 | 커스텀 객체 패널은 D26 으로 이미 열려 있다. 텍스트·도형 패널까지 열면 검증 경고 배선이 소비자 몫이 된다 |
| `pageFrame` `selectionOverlay` `resizeHandles` `canvasStage` `stageArea` | ❌ | **좌표계·포인터 라우팅·줌 앵커링.** 6장의 함정이 그대로 소비자 코드로 샌다 |

마지막 행이 D27 의 핵심이다. 캔버스를 열면 배율 이중 적용·`scrollLeft` 중복 가산 같은 실수가
소비자 코드에서 일어나고, 그 버그 리포트는 **우리가 재현할 수 없는 형태로** 도착한다.
...
### 20.22 예제 앱을 레포에 둔다 (2026.08.21)

`demo/` 는 `vite.demo.config.ts` 의 **별칭으로 소스를 직접 본다.** 고치면 즉시 반영되어 개발이
빠르지만, 그래서 **패키징 경로를 전혀 검증하지 못한다.**

| | `demo/` (:3100) | `examples/*` (:3101 · :3102) |
| --- | --- | --- |
| 해석 | 별칭 → `src/**` | **`node_modules` → `exports` 맵 → `dist/**`** |
| 반영 | 즉시 | `npm run build` 후 |
| 검증하는 것 | 동작 | **`exports` 맵 · 진입점 · `.d.ts` · peer** |

R10·R11 에서 잡은 버그 네 개가 전부 오른쪽 열에서만 드러났다.

| 버그 | 왜 `demo/` 가 못 잡나 |
| --- | --- |
| `postinstall` 이 tarball 없는 파일 실행 | 설치를 하지 않는다 |
| `SlotMap` 반공변 | 데모가 `as never` 캐스트로 우회했다 |
| `createPDFCanvasEditor` export 누락 | 데모가 상대 경로로 import 한다 |
| React `ref` 미충전 | 데모가 vanilla facade 를 직접 쓴다 |

그런데 그때 쓴 검증 앱은 스크래치패드에 있었다 — **커밋되지 않고 다음 세션에 사라진다.**
네 번 중 두 번을 놓친 이유가 그것이다.

#### 구조

```
examples/react/src/index.tsx    단일 파일. 편집기·뷰어·커스텀 타입 전부
examples/vue/src/App.vue        같음 (+ 슬롯 SFC 3개, 타입 1개)
scripts/dev-examples.mjs        세 서버를 함께 띄운다
```

`pdf-canvas-kit: "file:../.."` 로 참조한다. root 는 workspace 멤버가 아니므로 `"*"` 로는
registry 를 찾는다 — `file:` 이 심링크를 만들고, 그 심링크가 `exports` 맵을 타므로 실제
설치와 같은 경로가 된다. 브라우저 네트워크 탭에 `pdf-canvas-kit/dist/react/index.js` 가
그대로 보이는 것이 그 증거다.

`concurrently` 를 쓰지 않는다. 의존성 하나를 아끼려는 것이 아니라, 이 패키지는 남의 제품에
임베드되고 라이선스 정책이 좁아서 devDependency 도 같은 심사를 거쳐야 한다 — `spawn` 열 줄이면
되는 일에 그 비용을 낼 이유가 없다. `&` 를 쓰는 셸 한 줄은 Windows 에서 깨진다.

#### 예제가 문서보다 강한 지점

세 가지가 예제 안에서 **컴파일로 강제**된다.

| | |
| --- | --- |
| `defineObjectType<Answer, PublicAnswer>` | 두 번째 제네릭 없이는 뷰어 슬롯 타입이 거짓말한다 (§18.4) |
| `ref<PDFCanvasEditorRef>` | Vue 의 `expose` 는 타입을 남기지 않는다 (§17.2) |
| `configurePdfResources` 다섯 필드 | `workerSrc` 만 주면 한국어 글자가 조용히 사라진다 |

#### 확인된 것

브랜드 타입이 **spread 로 파생한 객체에도 유지된다.** 처음에 `as PublicPDFCanvasDoc` 캐스트를
넣었는데 린트가 "불필요" 라고 잡았다. 소비자가 응답을 반영한 문서를 캐스트 없이 그대로 다시
내려 줄 수 있다는 뜻이고, 두 예제 모두 그렇게 쓴다.
...
### 20.24 R12 — 커스터마이징 (2026.08.21)

D27 은 크롬 10개 조각을 슬롯으로 여는 계획이었다. **구현 직전에 방향을 바꿨다.**

#### 왜 슬롯을 버렸나

조각의 props 를 열어 보니 전부 `ReadSignal<T>` 였다.
...
React·Vue 로 넘기려면 조각마다 **plain 값 스냅샷 타입 + 변경 구독 배선**이 필요하다. 10개면
컨트롤러 표면의 절반을 공개하는 것과 같아진다 — D27 이 headless(b)를 미룬 바로 그 비용이다.

그런데 **실제 수요는 그게 아니었다.** "호스트 앱에 이미 자기 모달이 있고 우리 팝업만 다르게
생겼다" 는 것이고, 그건 UI 를 교체하는 문제가 아니라 **동작만 함수로 받으면** 끝난다.

#### 3단계

| 층 | 수단 | 해결하는 것 |
| --- | --- | --- |
| 값 | 토큰 `--pck-*` (54 → **75개**) | 색·간격·폭·모달 모양·버튼 패딩 |
| 규칙 | **`@layer pdf-canvas-kit`** | 배치·정렬처럼 값으로 안 되는 것 |
| 동작 | **다이얼로그 위임** | 호스트 모달을 그대로 쓰기 |

#### `@layer` 가 핵심이다

`editor.css` 전체를 레이어로 감쌌다. 캐스케이드 레이어 규칙은 레이어 밖 규칙에게 **특이도와
무관하게** 진다.

```css
.pck-modal { border-radius: 0; }   /* 소비자. 이 한 줄이 이긴다 */
```

레이어가 없으면 소비자는 우리보다 특이도를 높여야 하고, **우리가 선택자를 하나 늘리는 순간
조용히 깨진다.** `!important` 를 쓰라고 안내하는 것도 같은 문제를 미루는 것뿐이다.

토큰은 레이어에 넣지 않았다 — 토큰 오버라이드는 이미 의도된 첫 경로인데 한 단계 더 낮출
이유가 없다. 값 하나만 바꾸려는 호스트가 레이어 규칙을 몰라도 되게 둔다.

`verify:tarball` 에 구조 검사를 넣었다(레이어 존재·이름·토큰이 밖·레이어 밖 규칙 0개).
번들러가 레이어를 평탄화하면 증상이 "호스트 CSS 가 안 먹는다" 이고 원인 추적이 어렵다.

#### 다이얼로그 위임 (D31)

```
onRequestUpload   → 내장 업로드 팝업을 그리지 않는다
onRequestConfirm  → 내장 확인 팝업을 그리지 않는다
onImportStateChange → 진행률·오류를 호스트 UI 로
```

셸이 `controller.dialogsDelegated` 를 보고 렌더를 건너뛴다. 컨트롤러가 판단을 내려 주는
이유: 셸이 `props` 를 직접 읽으면 렌더 층이 prop 이름을 알게 되고 위임 조건이 두 곳에 생긴다.

facade 에 여섯 개를 노출했다 — `importFile` · `cancelImport` · `confirmPending` ·
`cancelPending` · `requestUpload` · `requestRemovePage`.

**위임했는데 `confirmPending()`·`cancelPending()` 을 부르지 않으면 대기 상태로 남는다.**
조용히 취소하지 않는다 — 사용자가 [삭제] 를 눌렀는데 아무 일도 없는 것과, 확인 없이
지워지는 것 중 어느 쪽도 낫지 않으므로 결정을 호스트에 남긴다.

#### 예제가 또 버그를 잡았다

`ConfirmRequest` · `ImportState` 를 `src/index.ts` 에서 export 하지 않았다. `examples/react` 의
타입체크가 즉시 잡았다 — **R11 의 `createPDFCanvasEditor` 누락과 같은 종류**이고, 그때는
검증 앱이 레포 밖에 있어 놓쳤다. D30 이 실제로 값을 낸 첫 사례다.

#### 함께 넣은 케이스

| 그룹 | 내용 |
| --- | --- |
| `shell — 다이얼로그 위임` | 위임 없으면 내장 팝업 · 위임하면 콜백만 · `confirmPending` 이 삭제 수행 · `cancelPending` 은 문서 불변 (5건) |
...
### 20.25 문구와 아이콘을 호스트에 넘긴다 (2026.08.21)

R12 직후 요청받은 것. "호스트에서 번역이 필요할 수 있다" 가 이유였다.

#### 문구 — prop 으로

`configureStrings()` 전역 함수는 이미 있었지만 **인스턴스 개념이 없다.** prop 이 라이브러리
관례다 (AG Grid 의 `localeText`, MUI 의 `localeText`).

```tsx
<PDFCanvasEditor strings={{ 'confirm.deletePage': 'Delete this page?' }} />
```

전역 표에 병합한다. 인스턴스 스코프로 만들려면 렌더 층 14개 파일(~80 호출)에 `t` 를 흘려야
하고, 반응형으로 만들려면 그 80곳의 렌더 경로를 signal 로 바꿔야 한다. 실제 요구는 "부팅 시
언어를 정한다" 이므로 그 비용을 내지 않았다 — **한계를 문서에 적었다.**

미번역 11건도 채웠다 (`upload.*` 7, `export.*` 4). `Google Drive` 는 고유명사라 남겼고,
`pages.title`(PAGES) · `inspector.title`(INSPECTOR) 는 와이어프레임 의도라 그대로 뒀다.

`confirm.deletePage` 가 "Answer Box·객체가 함께 삭제됩니다" 였다 — **D25 로 사라진 개념**이
문구에 남아 있었다. "객체가 함께 삭제됩니다" 로 고쳤다.

#### 아이콘 — 문자열만으로는 부족하다

처음에 글리프를 `strings` 의 `icon.*` 로 옮겼다. 그것만으로는 SVG 나 프레임워크 컴포넌트를
넣을 수 없다는 지적을 받았고, **커스텀 객체와 같은 3경로 패턴**으로 바꿨다.

| 우선순위 | 방법 | 무엇을 주는가 |
| --- | --- | --- |
| 1 | `icons` | `() => Node` — vanilla SVG |
| 2 | `renderIcon` (래퍼) | 프레임워크 컴포넌트 → portal |
| 3 | `strings` 의 `icon.*` | 글리프 (기본값) |

`src/dom/editor/icon.ts` 의 `icon(name)` 이 이 순서를 한 곳에서 결정한다. 사용처마다 분기를
두면 새 아이콘을 추가할 때 세 갈래를 다시 쓰게 되고, 한 곳만 빠뜨리면 그 아이콘만 조용히
커스터마이징이 안 된다.

기본이 유니코드 글리프인 이유: SVG 스프라이트를 내장하면 소비자가 그것을 교체할 수단이
**따로** 필요해지고 결국 아이콘 프레임워크를 요구하게 된다.

네 번째 길도 있다 — 아이콘 버튼에 `data-icon` 을 붙였으므로 CSS 로 배경 이미지를 줄 수 있다
(`@layer` 덕에 단일 선택자가 이긴다).

#### ⚠️ portal 키는 이름이 아니라 엘리먼트다
...
| 그룹 | 내용 |
| --- | --- |
| `shell — 문구 · 아이콘 오버라이드` | `strings` 반영 · 글리프 기본 · 글리프 교체 · **`icons` 가 글리프를 이긴다** · portal 통지 · **`icons` 가 있으면 portal 을 부르지 않는다** · 정리 시 null (7건) |

**우선순위를 일부러 뒤집어 케이스가 잡는 것을 확인했다.** 이 종류는 조용히 잘못 그려진다 —
소비자가 준 SVG 대신 글리프가 나오거나 그 반대인데, 에러가 없다.

#### 예제에 셋 다 넣었다

두 예제가 세 경로를 동시에 쓴다 — `renderIcon` 으로 undo·redo·zoom·back 을 컴포넌트로,
`icons` 로 close 를 vanilla SVG 로, `strings` 로 caret 글리프를 교체. `icons` 와 `renderIcon`
이 겹칠 때 어느 쪽이 이기는지 눈으로 보인다.

#### 썸네일 객체가 캔버스와 다른 색으로 보였다 (2026.08.21)

브라우저에서 잡힌 것. 캔버스의 빨간 도형이 썸네일에서는 보라색(accent)이었다 — 단색으로
그렸기 때문이다. 버그는 아니었지만 **"다른 객체" 처럼 읽혔다.**

모양을 단순화하는 것과 색을 바꾸는 것은 다른 이야기다. 타원·화살표를 사각형으로 그리는 것은
그 크기에서 모양이 읽히지 않으니 타당하지만, **색은 어느 객체인지 알아보는 단서**다.

실제 색을 인라인으로 받게 했다. 유형마다 색이 사는 곳이 달라 `objectColors()` 로 모았다 —
도형은 `style.fill`/`stroke`, 텍스트·커스텀은 박스 스타일, 마스크는 `fill` 하나다.

CSS 에서 유형별 색 강제를 지웠다. 인라인 값과 싸우면 어느 쪽이 이기는지 특이도로 결정돼
예측이 어려워진다 — 유형 구분은 **테두리 모양**으로만 한다(커스텀은 점선, 마스크는 투명).
색이 없는 객체는 인라인 값을 **빈 문자열**로 둬 토큰 기본값이 이기게 했다. `null` 을 넣으면
속성이 남아 토큰을 가린다.

`--pck-thumb-obj-custom` 토큰이 쓰이지 않게 되어 삭제했다.
