...
export const EDITOR_DEFAULTS = {
  ...
  zoom: {
    ...
    /**
     * Ctrl/Cmd + 휠(트랙패드 pinch 포함)의 **휠 픽셀당 지수 계수**.
     *
     * `scale *= wheelFactor ** -deltaY` 다. 지수를 쓰는 이유: 배율은 비율이므로 400% 에서의
     * 한 틱과 25% 에서의 한 틱이 같은 **비율**로 움직여야 한다. 선형으로 더하면 축소 쪽에서
     * 급격하고 확대 쪽에서 답답하다.
     *
     * 2026.08.21 에 1.0015 → 1.0025 로 올렸다(약 1.7배). 실측이 아니라 **손으로 만져 보고
     * 고른 값**이다 — 트랙패드 pinch 는 이벤트당 `deltaY` 가 1~10 밖에 안 되어 이전 값에서는
     * 한 번에 0.3~1.5% 밖에 움직이지 않았다.
     */
    wheelFactor: 1.0025,
    /**
     * 한 이벤트에서 인정하는 `deltaY` 상한(정규화 후).
     *
     * 없으면 `deltaMode: PAGE` 나 관성 스크롤의 급발진 한 방에 배율이 최소·최대로 튄다.
     * 120 이면 한 이벤트당 최대 26% 다. Chrome 의 마우스 휠 한 틱이 100 이라 평범한 휠은
     * 잘리지 않는다.
     */
    wheelMaxDelta: 120,
  },
  ...
  handles: {
    ...
    hitPx: 18,
    /**
     * 객체 경계에서 핸들 **중심**까지의 거리(CSS px). 핸들을 도형 밖으로 밀어낸다.
     *
     * 예전에는 핸들이 객체 경계에 딱 붙어 도형 위에 얹혔다. 얇은 객체 — 선·화살표는 박스가
     * 실제 두께로 조여진다(§21.1.1) — 에서는 핸들이 도형을 **완전히 덮어** 본체를 잡아 옮길
     * 수 없었다. 밖으로 빼면 가운데가 비어 드래그할 자리가 생긴다.
     *
     * 화면 px 이므로 배율과 무관하다. 좌표 계산이 아니라 **핸들 위치만** 옮기는 값이다 —
     * 리사이즈는 포인터 델타로 계산하므로 핸들이 어디 그려져도 결과가 같다.
     *
     * ⚠️ **`hitPx / 2 <= outsetPx` 를 유지한다.** 어기면 핸들의 히트 영역이 객체 본체까지
     * 넘어와, 얇은 객체에서 본체를 잡을 수 없다는 원래 문제로 되돌아간다.
     */
    outsetPx: 10,
  },
