 ...
 *
 * 모두 객체 수량 한도(페이지 30 / 문서 200)를 지킨다. 이 상한은 DOM 렌더 (캔버스는 canvas 가 아니라 DOM 절대 배치다)의 전제이고,
 * 드래그 중 rAF 코얼레싱을 쓰지 않는 판단도 여기에 기댄다.
 *
 * 이전 판은 한도를 Answer Box 에만 적용했다. 커스텀 객체는 소비자가 무엇이든 넣을 수 있으므로
 * (커스텀 객체는 소비자가 정의한다) 전체 객체 수로 바꿨다 — 성능 상한은 유형과 무관하다.
 */
 ...
 * ## `canRotate` 를 받는 이유
 *
 * 이전 판은 "Answer Box 는 회전하지 않는다"를 커맨드에 박아 뒀다. 뷰어 폼 요소가
 * 기울면 입력과 모바일 렌더가 깨지기 때문이다.
 *
 * 커스텀 객체 (커스텀 객체는 소비자가 정의한다)에서는 그 판단이 **소비자 것**이다 — `objectType.rotatable` 이 그것을
