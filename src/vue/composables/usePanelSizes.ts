...
export interface UsePanelSizes {
  pageListWidth: Ref<number>
  inspectorWidth: Ref<number>
  /** 드래그 중인 패널. 커서·오버레이 처리에 쓴다. */
  resizing: Ref<PanelId | null>
...
export function usePanelSizes(): UsePanelSizes {
  ...
  const pageListWidth = ref(stored.pageListWidth ?? LAYOUT_DEFAULTS.pageListWidthPx)
  const inspectorWidth = ref(stored.inspectorWidth ?? LAYOUT_DEFAULTS.inspectorWidthPx)
  const resizing = ref<PanelId | null>(null)
  ...
  watch([pageListWidth, inspectorWidth], persist)
