 ...
 *
 * 팬은 transform이 아니라 `scrollLeft`/`scrollTop` 을 움직인다. 스테이지가 스크롤 컨테이너이므로
...
export interface UsePanOptions {
  ...
  /** Space를 누르고 있는 동안 세워진다. 커서와 히트 테스트가 반응할 수 있게. */
  panArmed: Ref<boolean>
  /** 팬을 끈다. 모달이 열려 있을 때 등. */
  disabled?: Ref<boolean>
}

export interface UsePan {
  /** 팬 드래그가 진행 중이면 true. */
  panning: Ref<boolean>
}

/** 입력 필드 타이핑을 편집기 키가 가로채면 안 된다 (PLAN 11.4). */
...
export function usePan(options: UsePanOptions): UsePan {
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    // 이게 없으면 Space에 브라우저가 페이지를 스크롤해 팬과 충돌한다.
    e.preventDefault()
    panArmed.value = true
  }
  ...
  /** Space를 누른 채 포커스를 잃으면 스테이지가 계속 팬 대기 상태로 남는다. */
  function onBlur() {
    panArmed.value = false
    panning.value = false
  }
