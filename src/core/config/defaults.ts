...
export const EDITOR_DEFAULTS = {
  ...
  historyLimit: 100,
  /** 자동저장 디바운스와, 대기 중인 변경을 최대 얼마나 미룰지 (PLAN 12). */
  autosave: { debounceMs: 5_000, maxDelayMs: 30_000, retries: 3 },

  /** 줌 범위와 +/- 버튼이 밟는 프리셋 계단 (PLAN 6.4). */
  zoom: {
    min: 0.25,
    max: 4,
  ...
  /** 선택 핸들은 scale transform 밖에 그린다 (PLAN D5). */
  handles: {
    /** 화면상 크기(CSS px). 어떤 배율에서도 일정하다. */
    sizePx: 8,
    /** 포인터 히트 영역(CSS px). 잡기 편하도록 보이는 크기보다 크게 둔다. */
    hitPx: 14,
  },
...
export const RENDER_DEFAULTS = {
   ...
   *
   * ⚠️ **메모리 주의 (PLAN Q19)** — 이 값이 배경 이미지 용량을 직접 결정한다.
   ...
   * 좌표는 pt이므로 **해상도를 바꿔도 객체 위치는 움직이지 않는다** (PLAN 5.7).
   */
  targetPx: 1654,
