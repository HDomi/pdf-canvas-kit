...
- [11 종](#11-종)
- [모양별 기하](#모양별-기하)
- [⚠️ 선 · 화살표의 박스는 얇다](#️-선--화살표의-박스는-얇다)
...
대각선 화살표는 **회전**으로 만든다. 선 계열은 항상 수평으로 그려진다.
...
선 계열은 박스 높이가 그림에 아무 영향을 주지 않는다 — 선은 항상 좌측 중앙 → 우측 중앙이다.
그래서 박스 높이를 **실제로 차지하는 높이로 자동 조인다.**

| | 높이 |
| --- | --- |
| `line` | 선 두께 그대로 |
| `arrow` · `doubleArrow` | 화살촉 높이 (`선 두께 × 4`, 폭의 절반을 넘지 않음) |

조이는 시점 셋 — 크기 조절, 인스펙터에서 모양 변경, 테두리 두께 변경. 세로 중심은 유지하므로
그림은 제자리에 남고 박스만 달라붙는다.

```ts
import { lineShapeHeight, normalizeShapeRect } from '@h_domi/pdf-canvas-kit'

lineShapeHeight('line', 2, 100)   // 2
lineShapeHeight('arrow', 2, 100)  // 8
normalizeShapeRect(arrowObj, { x: 10, y: 60, w: 100, h: 120 })
// { x: 10, y: 118, w: 100, h: 4 }  ← 세로 중심 유지
```

그래서 사각형을 화살표로 바꾸면 **박스가 즉시 얇아진다.** 선택 핸들과 썸네일 자리 표시가 실제
도형에 붙는 것이 목적이다 — 그렇게 하지 않으면 얇은 선 하나가 큰 빈 상자 안에 남는다.

세로로는 크기를 조절할 수 없다. 두껍게 하려면 [테두리 두께] 를 키운다.

> 얇은 객체를 마우스로 집을 수 있도록 **히트 테스트에만** 여유를 준다
> (`EDITOR_DEFAULTS.minHitSize`, 8pt). 객체 크기는 바뀌지 않으므로 보이는 것은 그대로다.
> 마퀴 선택에는 여유가 없다 — 교차 판정이라 얇아도 걸리고, 여유를 주면 스치지도 않은 객체가
> 잡힌다.
...
arrowHeadSize(200, 8, 2) // 8 — 두께 × 4, 폭·높이를 넘지 않는다
```
