 ...
 *
 * 구 `src/vue/composables/usePageReorder.ts` 의 이식. 대응표는 `src/controller/README.md`.
 */
import { onCleanup, signal, type ReadSignal, type Signal } from '../dom/reactive'

export interface PageReorderOptions {
  /** 스크롤 컨테이너. 각 항목은 `[data-page-index]` 를 갖는다. */
  listEl: ReadSignal<HTMLElement | null>
...
export function createPageReorder(options: PageReorderOptions): PageReorder {
  const draggingIndex: Signal<number | null> = signal<number | null>(null)
  const dropIndex: Signal<number | null> = signal<number | null>(null)
