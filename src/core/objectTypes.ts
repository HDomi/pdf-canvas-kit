...
export interface ObjectTypeDef<Data = unknown> {
  ...
  defaultData: () => Data
  /**
   * 편집기에서 콘텐츠가 포인터 이벤트를 받을지 (PLAN D25).
   *
   * **기본은 `false`** — 콘텐츠에 `pointer-events: none` 이 걸려 클릭이 객체 선택·드래그로
   * 간다. 편집기는 배치·크기 조절을 위한 화면이고, 안쪽 입력이 포커스를 가로채면 객체를 옮길
   * 수 없다.
   *
   * `true` 로 켜면 그 타입만 콘텐츠가 이벤트를 먹는다. 그 객체는 테두리·핸들로만 선택된다.
   *
   * ⚠️ 뷰어에서는 이 값과 무관하게 **항상 콘텐츠가 먹는다.** 뷰어는 응답을 받는 화면이다.
   */
  interactive?: boolean
  /**
   * 회전을 허용할지. **기본은 `true`.**
   ...
   * 여기로 옮겼다. **`interactive: true` 인 타입은 대개 `rotatable: false` 가 맞다.**
   */
  rotatable?: boolean
  /**
