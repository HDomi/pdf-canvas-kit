...
#### ⚠️ 예외 — 핸들 크기·위치는 일부러 토큰이 아니다 (2026.08.21)
...
| --- | --- |
| `sizePx` (8) | **보이는** 크기 |
| `hitPx` (18) | **집는** 크기. 차이를 투명 테두리로 준다 |
| `outsetPx` (10) | 객체 경계에서 핸들 **중심**까지. 도형 밖으로 밀어낸다 |

⚠️ **`hitPx / 2 <= outsetPx` 를 유지한다.** 어기면 핸들의 히트 영역이 객체 본체까지 넘어와,
얇은 객체에서 본체를 잡을 수 없다는 원래 문제로 되돌아간다. 검증 케이스로 박아 두었다.

색(`--pck-handle-bg` · `--pck-handle-stroke`)은 토큰이다. 그쪽은 취향이고 겹침과 무관하다.

#### 핸들의 시각 — 히트 확장은 `padding` 이 아니라 투명 `border` 다 ★

보이는 크기와 집는 크기가 달라야 한다. 작아야 도형을 가리지 않고, 커야 잡힌다.

예전에는 `padding` 으로 넓히고 `border` 로 테두리를 그렸다. 박스 모델상 `border` 는 `padding`
**밖**이므로, 화면에는 작은 사각형 하나와 그것을 멀리서 감싸는 큰 테두리 사각형 둘이 보였다.

| 역할 | 수단 | 기준 박스 |
| --- | --- | --- |
| 히트 영역 | `border-color: transparent` + 인라인 `border-width` | border-box |
| 흰 배경 | `background-clip: content-box` | content-box |
| 파란 테두리 | **`box-shadow: inset`** | padding-box (= `padding: 0` 이므로 content-box) |

`inset` 그림자가 padding-box 기준이라는 점이 핵심이다. 그래서 테두리가 투명 테두리 밖이 아니라
**흰 사각형의 경계**에 그려진다.

의사요소(`::before`)로 그리는 방법도 있지만 거기에는 인라인 스타일을 줄 수 없어 크기를 CSS
변수로 흘려야 하고, 그러면 호스트가 핸들 크기를 바꿀 수 있게 된다 — 위 판단과 어긋난다.

⚠️ `.pck-handle--rotate` 에 `border-color` 를 주지 않는다. 테두리는 히트 확장용이므로 색을
주면 핸들에서 멀리 떨어진 큰 원이 하나 더 그려진다.

#### 종이 위 레이어는 다크 모드를 따라가지 않는다 ★ (2026.08.21)

D33 에서 팔레트 전체에 `light-dark()` 를 붙였는데 **종이 위 레이어를 걸러야 했다.** 판단 기준은
하나다: **무엇을 배경으로 깔고 그려지는가.** UI 크롬은 앱 배경 위, 이건 종이 위다.

종이는 PDF 를 래스터화한 흰 이미지라 항상 밝다. 다크 모드를 따라가면 흰 종이 위의 검은 핸들,
흰 반투명 박스 위의 흰 글자가 된다 — 실제로 핸들이 검은 사각형으로 보였고, 캔버스 커스텀
객체의 글자는 안 보이는 상태였다.

`--pck-paper-ink` · `--pck-paper-line` · `--pck-paper-danger` · `--pck-paper-accent` 넷을
기준으로 두고 종이 위 토큰이 그것을 참조한다. 리터럴을 여러 곳에 흩으면 다음에 하나를
빠뜨린다.
...
2026.08.21 에 **같은 함정이 도형 뷰에서 또 나왔다.** `shapeObjectView` 가 `const shape =
o().shape` 로 한 번 읽고 `when(() => shape === 'rect', …)` 로 분기했다. 반응형 읽기가 아니라
인스펙터에서 모양을 바꿔도 캔버스가 처음 모양에 굳었다. 뷰어는 문서 교체 시 노드를 새로
만들어 정상으로 보였고, 그래서 "편집기만 안 바뀐다" 로 나타났다 — **한쪽에서만 보이는 버그는
이 함정을 먼저 의심한다.**
...
이전에는 `interactive: true` 로 캔버스에서 직접 입력받는 길이 있었는데 **원리적으로 동작하지
않았다.** 콘텐츠가 이벤트를 받아도 페이지 프레임까지 버블링되고, 거기서 포인터 도구가
`preventDefault()` 를 부른다 — `pointerdown` 의 `preventDefault()` 는 **포커스 이동을 취소**한다.

`stopPropagation` 으로 고칠 수는 있지만 그러면 그 객체는 가운데를 끌어 옮길 수 없어진다.

**타입이 `renderInspector` 를 주지 않으면** 인스펙터가 "편집할 속성이 없다" 고 알린다.
빈 패널은 사용자가 원인을 알 수 없다.
...
## 18.2.1 ⚠️ 편집기와 뷰어를 나란히 두지 않는다

편집기는 3분할이고 `--pck-pagelist-width`(240px) + `--pck-inspector-width`(280px) 를 **고정으로**
먹는다. 화면 절반에 넣으면 캔버스에 남는 폭이 400px 대가 되어 못 쓴다 — D15 의 "Editor 는
데스크탑 전용" 은 창 크기가 아니라 **좁은 컨테이너를 주면 안 된다**는 제약이다.

두 화면이 필요하면 **탭으로 전환하고 둘 다 마운트해 둔다.**
...
**호스트가 바꾸고 싶은 것을 세 단계로 나눈다.** 슬롯 배선을 만들지 않은 이유가 이 표에 있다.

| 층 | 수단 | 왜 이 층인가 |
| --- | --- | --- |
| 값 | 토큰 `--pck-*` (78개) | 색·간격·폭은 값 하나 바꾸는 일이다 |
| 규칙 | **`@layer pdf-canvas-kit`** | 배치·정렬은 값으로 안 된다. 레이어가 특이도 싸움을 없앤다 |
| 동작 | **다이얼로그 위임** (D31) | 호스트 모달을 쓰려면 UI 가 아니라 함수가 필요하다 |

## 19.1 `@layer` — 소비자가 항상 이긴다

`editor.css` **전체**가 `@layer pdf-canvas-kit` 안에 있다. 캐스케이드 레이어 규칙은
레이어 밖 규칙에게 **특이도와 무관하게** 진다.

```css
/* 소비자. 이 한 줄이 우리 모든 규칙을 이긴다 */
.pck-modal { border-radius: 0; }
```

레이어가 없으면 소비자는 우리보다 특이도를 높여야 하고, 우리가 선택자를 하나 늘리면 조용히
깨진다. **토큰은 레이어에 넣지 않는다** — 토큰 오버라이드는 이미 의도된 첫 경로인데 한 단계
더 낮출 이유가 없다.

`npm run verify:tarball` 이 이 구조를 검사한다(레이어 존재·이름·토큰이 밖·레이어 밖 규칙 0개).
번들러가 레이어를 평탄화하면 증상이 "호스트 CSS 가 안 먹는다" 이고 원인 추적이 어렵다.

**클래스 이름은 공개 계약이다.** 바꾸면 breaking change 다.
...
이유: 셸이 `props` 를 직접 읽으면 렌더 층이 prop 이름을 알게 되고 조건이 두 곳에 생긴다.

| | |
| --- | --- |
| 진행률·오류 | `onImportStateChange({ progress, error })` |
| 문구 | `ConfirmRequest.message` — 이미 `strings.ts` 를 거친 상태로 온다 |
| 위험도 | `ConfirmRequest.danger` — 호스트가 강조 색을 고른다 |

⚠️ 위임했는데 `confirmPending()`·`cancelPending()` 을 부르지 않으면 그 동작은 **대기 상태로
남는다.** 조용히 취소하지 않는다.
...
렌더 층 14개 파일이 `text('key')` 를 부른다(~80곳). `strings` prop 을 받아 **전역 표에
병합**한다.

| | |
| --- | --- |
| 적용 시점 | 컨트롤러 생성 시 **1회.** `setProps` 에서 다시 읽지 않는다 |
| 스코프 | **전역.** 한 페이지에 언어가 다른 편집기 둘은 지원하지 않는다 |
| 타입 | `StringKey` — 오타가 컴파일 에러 |

인스턴스 스코프로 만들려면 조각 14개에 `t` 를 흘려야 하고, 반응형으로 만들려면 문구 80곳의
렌더 경로를 signal 로 바꿔야 한다. 실제 요구(호스트가 부팅 시 언어를 정한다)에 그 비용은
과하다. **런타임 언어 전환은 컴포넌트 재마운트로 처리한다.**
...
분기를 두면 새 아이콘을 추가할 때 세 갈래를 다시 쓰게 되고, 한 곳만 빠뜨리면 그 아이콘만
조용히 커스터마이징이 안 된다.

기본이 유니코드 글리프인 이유: SVG 스프라이트를 내장하면 소비자가 그것을 교체할 수단이
**따로** 필요해지고 결국 아이콘 프레임워크를 요구하게 된다. 글리프는 문구와 같은 경로로
덮어쓸 수 있어 계약이 하나 줄어든다.

### ⚠️ 아이콘 portal 키는 이름이 아니라 엘리먼트다

같은 아이콘이 여러 곳에 나온다 — `icon.remove`(×)는 인스펙터 필드마다 하나씩이다. 래퍼가
이름을 키로 쓰면 나중 것이 앞의 것을 덮어써 **하나만 그려진다.** 두 래퍼 모두
`Map<HTMLElement, IconName>` 을 쓴다.

### ⚠️ `setIconMount` 는 인스턴스가 아니라 모듈에 붙는다

`icon()` 이 인스펙터 필드처럼 렌더 층 깊은 곳에서도 불리므로 prop 을 흘리는 비용이 크다.
facade 가 편집기를 만들 때 세우고 정리 시 지운다. **편집기가 둘이고 둘 중 하나만
`renderIcon` 을 주면 나중에 만든 쪽의 설정이 이긴다** — 그 요구가 오면 인스턴스 스코프로
바꾼다.

### 새 아이콘을 추가할 때

1. `IconName` 에 이름을 넣는다 (`core/config/icons.ts`)
2. `DEFAULT_STRINGS` 에 `icon.<name>` 글리프를 넣는다 — 없으면 아무것도 그려지지 않는다
3. 사용처에서 `icon(name)` 을 부르고 버튼에 `data-icon` 을 붙인다

## 19.3 왜 크롬 슬롯을 만들지 않았나

D27 은 크롬 10개 조각을 슬롯으로 여는 계획이었다. 구현 직전에 방향을 바꿨다.

조각의 props 가 전부 `ReadSignal<T>` 다. React·Vue 로 넘기려면 조각마다 **plain 값 스냅샷
타입 + 구독 배선**이 필요하고, 10개면 컨트롤러 표면의 절반을 공개하는 것과 같아진다 —
D27 이 headless 를 미룬 그 비용이다.
...
## 21. 도형과 글꼴 (2026.08.21)

### 21.1 도형 정점은 core 에 있다

`ShapeKind` 가 4종에서 **11종**으로 늘었다(`triangle` `diamond` `pentagon` `hexagon` `star`
`cross` `doubleArrow` 추가). 다각형 계열의 정점은 `core/geometry/shapes.ts` 에 있고 뷰는
그 문자열을 `<polygon points>` 에 넣기만 한다.

렌더 층에 두지 않은 이유: 정점은 순수 계산이라 **브라우저 없이 검증할 수 있다.** 뷰 안에
두면 별 모양이 맞는지 보려고 화면을 띄워야 하고, `npm run checks` 가 커버하지 못하는 자리가
생긴다.

| | |
| --- | --- |
| 단위 | **pt.** 배율은 페이지 컨테이너의 `transform` 한 곳에만 있다(§6.2) |
| inset | `strokeWidth / 2`. stroke 가 경로 중앙에 그려지므로 정점을 안으로 민다 |
| 오프셋 방식 | 정확한 오프셋 곡선이 아니라 **바운딩 박스 축소**다. 뾰족한 꼭짓점에서 결과가 다르지만 박스 안에 확실히 들어가고 계산이 한 줄이다 |
| 좌표 정밀도 | 소수 3자리. `Math.cos` 잔재가 DOM 속성에 새면 값 비교가 매번 달라진다 |
| miter | 다각형은 `stroke-linejoin: round`. 별의 다섯 끝이 두께에 비례해 침처럼 튀어나온다 |

`ShapeKind` 는 **추가만** 한다. 지우면 그 도형으로 저장된 기존 문서가 해석되지 않는다.

### 21.1.1 ★ 선 계열은 박스 높이를 스스로 정한다 (2026.08.21)

선은 박스의 좌측 중앙 → 우측 중앙에 그려지므로 **박스 높이가 그림에 아무 영향을 주지 않는다.**
그대로 두면 200pt 박스 안에 1pt 선 하나가 떠 있고, 선택 핸들과 썸네일 자리 표시가 그 빈 박스를
따라간다. 실제로 그렇게 보였다.

`normalizeShapeRect(obj, rect)` 가 선 계열의 높이만 실제 크기로 조인다. **세로 중심을
유지한다** — 높이만 줄이면 선이 위로 튀어 올라 "핸들을 놨는데 도형이 움직였다" 가 된다.

부르는 곳 셋. 하나라도 빠지면 그 경로에서만 빈 상자가 남는다.

| 어디 | 왜 |
| --- | --- |
| `pointerMachine` 의 resize | **미리보기부터** 정규화해야 한다. 커밋에서만 하면 손을 뗀 순간 박스가 튄다 |
| `transformObjects` | 방향키 이동처럼 포인터를 거치지 않는 경로 |
| `updateObject` | 인스펙터에서 모양·두께를 바꿀 때. 사각형→화살표가 주 경로다 |

화살촉 크기는 `arrowHeadSize(w, h, strokeWidth)` 하나로 계산한다. 박스 높이를 정하는 식과
**같은 식**이어야 촉이 박스에 정확히 들어찬다 — `<svg>` 의 기본 `overflow` 는 hidden 이라
넘치면 잘려서 촉이 뭉툭해 보인다. 두 곳에 적으면 한쪽만 고쳐 그 증상이 난다.

세로 리사이즈는 사실상 불가능해진다. 두께는 [테두리 두께] 로 바꾼다.

⚠️ **얇아진 만큼 집기가 어려워진다.** `hitTestObject` 가 `EDITOR_DEFAULTS.minHitSize`(8pt)
보다 얇은 축만 그 값까지 벌려 판정한다. 객체 크기는 바꾸지 않으므로 보이는 것은 실제
크기다 — 보이는 것과 집히는 것이 다른 편이, 빈 상자가 그려지는 것보다 낫다. 마퀴
(`pickObjectsInRect`)에는 여유가 없다: 교차 판정이라 얇아도 걸리고, 여유를 주면 스치지도 않은
객체가 잡힌다.

### 21.2 선택기가 줄바꿈한다

11종이 인스펙터 폭(280px)을 넘긴다. `.pck-segmented` 의 구분선을 버튼 `border-right` 에서
**`gap: 1px` + 컨테이너 배경**으로 바꿨다 — 그 방식으로는 줄 사이 구분선과 마지막 줄의 남는
테두리를 동시에 처리할 수 없었다. 버튼이 `flex-grow: 1` 이라 마지막 줄의 남는 폭도 항목들이
채우므로 선 색 바탕이 드러나지 않는다.

라벨은 `icon.shape.*` **문구**고, 버튼에 `data-shape` 속성이 있다. `⬠`(U+2B20) ·
`⬡`(U+2B21) 은 폰트 커버리지가 좁아 두부(□)가 될 수 있으므로 열어 두는 것이 필수다.

### 21.3 웹폰트 파일을 싣지 않는다 ★

`core/config/fonts.ts` 가 가진 것은 **가족 이름뿐**이다. `.woff2` 도 `@font-face` 도 없다.
...
기본 목록의 한글 폰트는 전부 OFL 1.1(상용 이용 무료)로 골랐지만 **불러오는 것은 호스트의
몫**이고, 그 사실을 인스펙터에도 한 줄로 적어 둔다(`inspector.fontFamilyNote`).

문서에 저장되는 값은 **CSS `font-family` 스택 문자열**이다. 폰트 id 를 저장하면 그것을
해석할 표가 어디에도 없고, 스택이면 다른 앱·다른 기기에서 그 자리에서 폴백이 결정된다.

`configureFonts()` 는 병합이 아니라 **교체**다. 앱이 실제로 불러오는 폰트만 보여야 한다 —
없는 폰트를 고르면 폴백으로 그려져 "왜 안 바뀌지" 가 된다. 빈 배열이면 인스펙터에서 항목이
사라진다.

`strings` · `icons` 와 같이 모듈 수준 상태다(§19.4). 인스턴스별 목록 요구는 아직 없다.

### 21.4 좁은 폭에서 뷰어가 밀리지 않게

`.pck-viewer` 에 `min-width: 0` 을 준다. flex 자식의 기본 `min-width` 는 `auto` — 콘텐츠의
min-content 폭이다. 페이지 프레임은 폭이 px 로 고정돼 있으므로 그 값이 하한이 되어 컨테이너가
좁아져도 뷰어가 줄지 않고 호스트 레이아웃을 가로로 밀어낸다. 그러면 폭 측정값이 굳어 배율도
그대로 남는다(§18.3).

페이지 스택은 `align-items: safe center` 다. 페이지가 컨테이너보다 넓어지는 순간 `center` 는
넘치는 절반을 **왼쪽으로** 밀어내는데, `overflow-x: hidden` 이라 그쪽은 스크롤로도 되돌릴 수
없어 왼쪽이 잘린 채 남는다. 미지원 브라우저를 위해 앞줄에 `center` 를 함께 적는다.

⚠️ **좁은 폭 증상의 첫 용의자는 호스트 상단 바다.** 2026.08.21 에 "650px 이하에서 뷰어가
깨진다" 로 보고된 것의 실제 원인은 예제 앱의 `.ex-bar` 가 `flex-wrap` 없이 한 줄이어서 가로로
넘치고 `<body>` 에 가로 스크롤이 생긴 것이었다.

---

## 22. ⚠️ 폼 컨트롤은 시스템 색을 쓰지 않는다 (2026.08.21)

UA 스타일시트가 `<button>` 에 `color: ButtonText` 를, 입력에 `color: FieldText` 를 준다.
**부모의 `color` 를 상속하는 것이 아니라 시스템 색을 쓴다는 뜻이다.**

§20.3 에서 `color-scheme: light dark` 를 켠 뒤로 그 값이 OS 모드를 따라간다. 그래서 팔레트를
밝은 값으로 하드코딩한 호스트 테마에서 **흰 버튼에 흰 글자**가 됐다 — 예제 앱에서 툴바와
아이콘 버튼의 글자가 통째로 사라졌다. 인스펙터 라벨은 `<span>` 이라 정상이었고, 그래서 증상이
"버튼만 비어 보인다" 로 갈렸다.

`color: var(--pck-ink)` 를 버튼 공통 규칙에 넣었다. `npm run verify:tarball` 이 컨트롤 클래스
7개가 각각 `color` 를 명시하는지 검사한다 — `.pck-tool.is-active` 처럼 상태 선택자가 주는
`color` 는 세지 않는다. 그걸 통과로 세면 검사가 아무것도 잡지 못한다.

### 호스트에게 남는 몫

체크박스 · 드롭다운 · 색 선택기처럼 **브라우저가 직접 그리는** 컨트롤은 CSS 로 색을 줄 수
없다. `color-scheme` 만이 그것을 정한다. 그래서 밝은 팔레트로 고정하는 테마는
`color-scheme: light` 를 함께 줘야 한다 — 두 예제의 `theme.css` 가 그 형태다.

패키지가 대신 정해 주지 않는 이유: 토큰을 덮어썼는지 알 수 없다. 기본값이 `light dark` 인 것은
**토큰도 함께 따라가기 때문에** 짝이 맞고, 토큰만 밝게 고정한 쪽에서 짝이 깨진다. 그 판단은
팔레트를 정한 사람이 해야 한다.
