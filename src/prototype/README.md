...
3. `PDFCanvasEditor.vue` 의 `manual-save` 를 `export` 로 되돌리고
...
| 키 | 내용 |
| --- | --- |
| `pdf-canvas-kit.images` | `{ [assetId]: base64 data URL }` — 페이지 배경 이미지 |
| `pdf-canvas-kit.doc` | 문서 JSON. 배경 `url` 은 `pck-local:<assetId>` 참조로 바뀐다 |

Viewer가 두 키를 읽어 `pck-local:` 참조를 이미지 키의 base64로 되돌려 조합한다.
